...
export default function Main() {
  ...
  var clarifyProject = async (segment) => {
     try {
        const response = await fetch(`http://127.0.0.1:8000/api/research-query/clarify/${segment.project_id}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  var approveProject = async (segment) => {
     try {
        const response = await fetch(`http://127.0.0.1:8000/api/research-query/approve/${segment.project_id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
  ...
  var loadDetails = async (segment) => {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/research-query/status/${segment.project_id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/research-query/result/${segment.project_id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
          ...
          <div className="flex-1 w-[1050px] mainPanel">            
            <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6 mt-1">
              ...
              <TabsContent value="details">                   
                  <DeepResearcher view={"list"} data={detailsData}/>
              </TabsContent>
              <TabsContent value="card">                  
                  <DeepResearcher view={"card"} data={detailsData}/>
              </TabsContent>
              </>
              ) : null)
              }
